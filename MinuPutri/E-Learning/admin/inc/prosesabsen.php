  ...
  if($_GET['kategori']=="absen_siswa"){
      if($_POST['aksi']=="absen_baru"){
        foreach ($_POST['id_siswa'] as $id_siswa ) {
          $query_absen=mysqli_query($konek,"insert into absen_siswa(tanggal_absen,keterangan,id_siswa)
          values('$_POST[tanggal_absen]','$_POST[kehadiran]',$id_siswa);");
        }
      }else if($_POST['aksi']=="edit_absen"){
        foreach ($_POST['id_siswa'] as $id_siswa ) {
          $query_absen=mysqli_query($konek,"UPDATE absen_siswa SET tanggal_absen = '$_POST[tanggal_absen]', keterangan = '$_POST[kehadiran]'
          WHERE id_siswa = $id_siswa and tgl_absen='$_POST[tanggal_absen]';");
        }
      }else {
        foreach ($_POST['id_siswa'] as $id_siswa ) {
          $query_absen=mysqli_query($konek,"DELETE FROM absen_siswa WHERE id_siswa = $id_siswa and tanggal_absen='$_POST[tgl_absen]'");
        }
      }
